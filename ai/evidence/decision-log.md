...
2026-04-23 | Optimistic locking on all mutable ECN endpoints via If-Unmodified-Since / updated_at; 409 on stale write; 428 if header absent | [ADR-008](../../decisions/ADR-008-optimistic-locking-concurrent-ecn-edits.md)
2026-04-23 | Lesson learned LL-001: concurrent DRAFT edit gap missed in Phase 0 expert review; 5-point data mutation checklist added to CLAUDE.md | [LL-001](../evidence/lesson-learned-LL-001.md)
2026-07-16 | PLM v3 source re-verification: W-0 already fixed, D5's manufacturer-synonym claim misattributed to PLM, PLM compare engine has 3 real defects — folded into ADR-012 | [ADR-012](../../decisions/ADR-012-bom-module-iteration-2-scope-and-comparison-engine.md)
2026-07-20 | BOM Module Iteration 2 scope cut (~11.9wk → ~10.4-10.8wk), Slice E0 split out, external dependency checkpoints, R5/R6/R7 mitigations, D2 retention policy | [ADR-012](../../decisions/ADR-012-bom-module-iteration-2-scope-and-comparison-engine.md)
2026-07-22 | Parallel execution model for Iteration 2: worktrees oskar-bom-slice-ab (Slices A/B) + oskar-bom-slice-c (Slice C), converging for Slice D | [ADR-012](../../decisions/ADR-012-bom-module-iteration-2-scope-and-comparison-engine.md)
2026-08-21 | AD group nesting: Business Function groups nest into ecn-initiator/ecn-approver, ecn-doc-controller stays direct; get_groups() switches from memberOf to LDAP_MATCHING_RULE_IN_CHAIN; LDAPDirectoryError separates directory faults from "no groups" (503, never 401/403); scripts/ldap_verify.py added as preflight check #5 | [ADR-013](../../decisions/ADR-013-ad-group-nesting-and-directory-fault-handling.md)
2026-08-24 | BOM changes become self-contained (Option A): ecn_bom_changes gains ecn_id + parent_item_number NOT NULL, ecn_item_id nullable. Stargile verified — ZECNBOMS has no item FK, and Stargile's own "must exist in ZECNITMN" check was written then commented out. BOM-only ECNs confirmed common by user feedback. Movex-existence (MITMAS) check adopted at authoring time | [ADR-014](../../decisions/ADR-014-bom-changes-require-parent-item-on-ecn.md)
2026-08-27 | Workbench Planning Module (production scheduling) re-confirmed deferred, formally superseding an orphaned April draft that was never closed out (no ADR, risk register never updated). Distinct from and not to be confused with the Purchase Planner Workbench module now being scoped — no code/data overlap found. Re-trigger: once Purchase Planner Workbench ships, re-verify M3 tables + a live PMS100MI call | [ADR-015](../../decisions/ADR-015-workbench-planning-module-scope-deferred.md)
