...
| Stock code autofill — DigiKey/Nexar (S3-3) | Backend ✅; adapters wired; creds live | → Sprint 9 (S9-6) |
...
## Sprint 9 — Digest, Lineage, Mounting Type, Recovery Panel, Reassignment, Autofill UI ⏳ PLANNED

> Scope locked 2026-07-10, extended 2026-07-13 (S9-6 added once DigiKey production creds
> unblocked S3-3), extended again 2026-07-16 (S9-8 added — engineering team flagged at the
> weekly meeting that routing and MPN changes need the same bulk-upload path items already
> have). Ordered quick-wins-first: S9-1 and S9-2 are backend/frontend-only against
> existing data; S9-4 and S9-5 need new backend (endpoint/trigger) design.

| # | Task | Scope | File(s) | Est. |
|---|------|-------|---------|------|
| S9-1 | ✅ Facility-scoped ECN digest — `_fetch_open_ecns(facility)` and `_fetch_digest_recipients(facility)` now take a facility param; `_send_ecn_digest_async` fans out one digest per facility (via new `_fetch_open_facilities()`) to that facility's DCs only | Backend only | `src/tasks/ecn_notifications.py`, `tests/tasks/test_ecn_notifications.py` | Done 2026-07-10 |
| S9-2 | ✅ ECN revision lineage UI — `GET /api/v1/ecn/{id}/history` returns the full `ecn_transition_history` chain (oldest first) with a `chain_valid` flag verifying each row's `sha256_prev` links to the previous row's `sha256_self`; `RevisionLineagePanel.tsx` renders it as a collapsible vertical timeline with a chain-verified/broken badge | Backend + Frontend | `src/services/ecn/service.py` (`get_history`), `src/routers/ecn_core.py`, `frontend/src/components/ecn/RevisionLineagePanel.tsx` | Done 2026-07-13 |
| S9-3 | ✅ Part mounting type — `mounting_type` column on `ecn_items` (TH \| SMD \| MECHANICAL \| OTHER — MECHANICAL added beyond original scope, matching the original Stargile SO050/SO100/SO160 routing categories); DigiKey adapter extracts it from the `Parameters` array; `/parts/autofill` applies it only when not already set (won't clobber manual entry); dropdown in `ECNItemPanel` Classification section. Side effect: found and fixed two pre-existing DigiKey adapter production bugs (circuit breaker used an unsupported context-manager protocol on pybreaker; OAuth token URL was hardcoded to production even when sandbox was configured, meaning sandbox credentials could never authenticate) — both confirmed fixed via live sandbox OAuth test. | Migration + Backend + Frontend | `alembic/versions/0024_ecn_items_mounting_type.py`, `src/services/ecn/items.py`, `src/routers/ecn_schemas.py`, `src/routers/parts.py`, `src/adapters/suppliers/digikey.py`, `frontend/src/components/ecn/ECNItemPanel.tsx` | Done 2026-07-13 |
| S9-4 | ✅ DC Movex Write Recovery panel — `GET /admin/movex-outbox` (defaults to failed+abandoned, filterable by state/facility) and `POST /admin/movex-outbox/{id}/retry` (resets attempt_count to 0 to satisfy `chk_outbox_not_requeued` and bypass `process_outbox_entry`'s terminal-state skip, then re-dispatches via Celery). New "Movex Write Recovery" tab on Admin page with 30s auto-refresh and a confirm modal. Live-verified end-to-end: abandoned→pending→processing→failed round trip against the real Celery worker. **Side addendum:** Admin page restructured from one long scrolling page into a left-nav 4-tab layout (Role Assignments / Movex Recovery / Customer SE-PM Defaults / AD Groups) — pure frontend refactor, no component logic changed. | Backend + Frontend | `src/services/admin.py`, `src/routers/admin.py`, `frontend/src/pages/AdminPage.tsx`, `frontend/src/api/ecn.ts` | Done 2026-07-13 |
...
| S9-6 | ✅ Stock code autofill UI (S3-3 unblocked) — discovered `/parts/autofill` always wrote to `ecn_items` immediately (no preview mode existed), so added `dry_run` to `AutofillRequest`/new `AutofillPreviewResponse` — runs the same DigiKey/Nexar/Movex lookups but returns the result without calling `svc.update_item()`. dry_run gracefully degrades on Movex errors (circuit breaker/404/connect/timeout all swallowed, logged, not raised) since a Movex outage shouldn't block showing supplier data that already succeeded — non-dry_run path keeps its original hard-failure behavior unchanged. "Autofill from MPN" button added to `ECNItemPanel` Details tab (gated on a default MPN existing), preview card shows description/mounting_type/unit_of_measure with Discard/Apply-to-form actions — nothing writes until the user applies. **Side find:** fixed a real dormant bug in `SupplierChain._cache_get` — `json.loads()` was called on a `jsonb` column that asyncpg already deserializes to a `dict`, crashing on every real cache hit (only surfaced once this session's DigiKey testing populated the cache for the first time ever). Visually verified end-to-end via Playwright against the live production DigiKey API (`LM741CN/NOPB` → real product data → applied to item_name + mounting_type). 12 new tests (6 dry_run, 5 Movex-degradation, 1 cache-bug regression). | Frontend + Backend | `frontend/src/components/ecn/ECNItemPanel.tsx`, `frontend/src/api/ecn.ts`, `src/routers/parts.py`, `src/adapters/suppliers/chain.py` | Done 2026-07-14 |
| S9-7 | ✅ Movex write-path bugs found live-testing `dc_approve → Movex write` with a routing-op ECN. **Bug 1 (critical):** every write method in `MovexRestAdapter` (`create_product`, `add_bom_component`, `delete_bom_component`, `update_routing_operation`, `add_routing_operation`, `add_item_alias`) called `/mi/{program}/{transaction}` — a path prefix that doesn't exist on movex-rest-api's real routes (confirmed via its `TransactionController.cs` `[Route]`/`[HttpPost]` attributes: correct path is `/{program}/{transaction}`). **No Oskar Movex write has ever been able to reach movex-rest-api.** **Bug 2:** `add_routing_operation`/`update_routing_operation` were missing required `FACI`/`STRT` fields and sent an invalid `opds` field (PDS002MI.AddOperation/UpdateOperation only accept PLGR/PITI, no description). **Bug 3:** `_queue_routing_operations_outbox` didn't select `item_number`/`facility` into `mi_params` at all. All three fixed. ECN-2026-D-0010 (facility D, routing op, real routing-changes ECN) confirmed the fixed queuing produces correct `mi_params` end-to-end through `dc_approve`. **movex-rest-api field-offset bug (blocked a true live M3 write) fixed and confirmed 2026-07-15** — see [[movex-rest-api-pds002mi-field-offsets-corrupt-response-data]] in the Knowledge-Management vault: root cause was wrong field byte-offsets in `PDS002MI.json`'s transaction config, fixed on the movex-rest-api side (not Oskar). Re-tested live after the fix: real routing operation (op 998, `FCT`, 3.25 min) written to real item `LFAM050001` and confirmed via independent `LstOperation` readback — also surfaced the `PITI` ×100 scale factor (M3 stores run time as minutes × 100), fixed in `MovexRestAdapter`. | Backend | `src/adapters/erp/movex.py`, `src/services/ecn/workflow.py`, `tests/adapters/test_movex_routing_operations.py`, `tests/integration/test_ecn_workflow.py` | Done 2026-07-14 (Oskar-side); movex-rest-api fix confirmed 2026-07-15 |
| S9-8 | ✅ Bulk routing operation + MPN upload — engineering team asked for the same bulk-upload path items already have (`/items/bulk`), for routing and MPN changes too. `POST /ecn/{id}/routing/bulk` is a multi-item, ECN-wide template (Item No, Operation No, Operation Description, Work Centre, Run Time, Setup Time, Change Type) — rows can share one Item No (a full routing) or span many items in one file; each row's Item No must already exist on the ECN (no item creation). `POST /ecn/{id}/mpns/bulk` uses the real CAD BOM export shape (C P/N, Manufacturer 1/2, Manufacturer 1/2 Part Number) instead of an invented Oskar-native format, since engineers already produce this file from their CAD tool — each row expands to 1-2 MPN inserts (Manufacturer 1 = default, Manufacturer 2 = optional alternate). Both DRAFT-only, matching `/items/bulk`. Extracted the original bulk-items endpoint's xlsx/csv parsing into a shared `BulkUploadSpec`/`parse_bulk_upload` (`src/routers/bulk_upload.py`) so all three bulk endpoints share one parser — refactored `bulk_create_items` onto it with zero test regressions. 38 backend tests passing (25 new, 13 existing). Live-verified end-to-end via Playwright against the real dev DB and browser: a 10-row routing upload split correctly across 2 items, a 2-row CAD-BOM MPN upload correctly expanding to 3 MPNs (1 primary + 1 primary/1 alternate pair). **Deferred to Iteration 2** (see plan backlog items L/M): auto-resolving MPN rows with a blank C P/N against Movex (`GET /api/mitpop/search`) then DigiKey instead of hard-rejecting them; a single shared frontend/backend column-spec config to remove the hand-duplicated column lists across all three bulk templates. | Backend + Frontend | `src/routers/bulk_upload.py`, `src/routers/ecn_routing.py`, `src/routers/ecn_items.py`, `src/routers/ecn_schemas.py`, `src/services/ecn/items.py`, `frontend/src/lib/ecn-routing-csv-parser.ts`, `frontend/src/lib/ecn-mpn-csv-parser.ts`, `frontend/src/components/ecn/RoutingUploadDrawer.tsx`, `frontend/src/components/ecn/MPNUploadDrawer.tsx`, `frontend/src/api/ecn.ts`, `frontend/src/pages/ECNDetailPage.tsx`, `tests/routers/test_ecn_routing_bulk.py`, `tests/routers/test_ecn_mpns_bulk.py` | Done 2026-07-16 |

**Total estimate:** ~7-8 dev days, single sprint.
...
| I2-10 | ✅ J — Part TH/SMD mounting type — delivered early as S9-3 (added MECHANICAL beyond original TH\|SMD\|OTHER scope) | Sprint 8 (delivered Sprint 9) | Done 2026-07-13, see S9-3 |
...
| I2-15 | Movex BOM-finder dialog on `BOMComparePage` — search-by-item/customer picker for the "live ERP item" compare source, matching the finder pattern PLM's `/bomcomparison` uses. Backend (`GET /api/v1/bom/{item}`) already supports it; no picker UI exists yet — users must know the item number. **Partially delivered 2026-08-25 (ADR-014):** the browse-by-item-number half now exists in `AddBomChangeDrawer.tsx` (enter a parent → live BOM lines → click a line to author a change against it, old values prefilled). Still open: (a) search-as-you-type item finder, which needs a backend route over the adapter's existing `search_items` — nothing exposes it today; (b) porting the browser to `BOMComparePage` itself. | Slice D delivery gap, 2026-08-06 | `frontend/src/pages/BOMComparePage.tsx`, `frontend/src/components/ecn/AddBomChangeDrawer.tsx` |
...
| I2-18 | **Infra bug** — `tests/integration/conftest.py`'s `db_session` fixture hangs on the SECOND consecutive test in any file whose tests call `ECNService.create()` (or otherwise INSERT a row with a uniquely-constrained column). Root cause: on this Windows/Python 3.14 dev machine, the fixture's background-thread teardown fails mid-rollback with `RuntimeError: Event loop is closed` (Proactor event-loop connection-write races against thread/loop shutdown) — the Postgres session is never cleanly closed, so the next test's INSERT blocks indefinitely on a unique-constraint check against the still-open transaction. Reproduces identically on `master` (confirmed via `tests/integration/test_ecn_service.py`, pre-existing, unrelated to any Slice E code) — not a Docker/Rancher issue, the test-db container stays healthy throughout. Workaround in use for Slice E: run affected integration test files one test at a time. Needs a real fix in the fixture (e.g. explicit connection-level `terminate()` fallback on teardown, or running the teardown thread under an explicit non-Proactor event loop) before any integration suite with 2+ `ECNService.create()`-calling tests can run as a single `pytest` invocation again. | Found while verifying Slice E, 2026-08-07 | `tests/integration/conftest.py` (`db_session` fixture); affects every integration test file that creates more than one ECN across separate tests |
| I2-19 | ✅ **RESOLVED 2026-08-11 — switched close mechanism, no longer blocked on movex-rest-api.** W-1 (`PDS002MI.UpdateComponent`) shipped 2026-08-11 and was live-tested against real CONO=300 data: key resolution (CONO/FACI/PRNO/STRT/MSEQ/OPNO/FDAT) and a general field update (CNQT) both worked correctly, but `TDAT` — the one field that write path existed to set — did not persist (reported success, unchanged on read-back, reproduced 3x across payload variations; root cause traced as far as possible to `PDS002BE`, not visible in available RPG source). Rather than wait on a movex-rest-api-side fix, the movex-rest-api team suggested a Delete+AddComponent close/reopen pattern instead of UpdateComponent/TDAT. Verified against Stargile's real source (`ProcessBOMLineRule.java`, `com.startronics.ecn.process.rules`) that this is in fact **closer to Stargile's own proven pattern**: Stargile's live BOM-apply engine never called `UpdateComponent`/TDAT for BOM lines at all — CHANGE there is a plain `addComponent` at the ECN's effective date (`ZECNHEAD.EHFDAT`), and DELETE uses its own add-then-delete trick. `update_bom_component`/`UpdateComponent` is retired (kept as reference-only dead code, not on any dispatch table). `_queue_bom_changes_outbox` (`workflow.py`) now queues `PDS002MI.Delete` for the close step (both DELETE and the close-half of CHANGE) instead of `UpdateComponent`; the CHANGE add step's `FDAT` is the change's own `from_date` (already captured, no new UI/data needed — matches Stargile's EHFDAT-sourced model). Live-tested end-to-end 2026-08-11 against CONO=300 item `LFAM050001` (MSEQ 150): Delete removed the line (confirmed absent via read-back), AddComponent recreated it with a new FDAT (20240118→20260901) and CNQT (1.0→2.0), both changes confirmed via read-back — proves the full CHANGE pattern works where UpdateComponent/TDAT did not. Test line restored to exact baseline afterward; BOM confirmed back to its clean 11-line state. | Slice E, R2 risk mitigation, resolved 2026-08-11 | `src/services/ecn/workflow.py` (`_queue_bom_changes_outbox` — DELETE/CHANGE branches now use `PDS002MI.Delete`), `src/tasks/movex_outbox.py` (`_dispatch_mi_call` — `UpdateComponent` removed from dispatch table, guard removed since it's no longer reachable), `src/adapters/erp/movex.py` (`update_bom_component` docstring marks it dead code), `tests/services/ecn/test_queue_bom_changes_outbox.py`, `tests/integration/test_queue_bom_changes_outbox.py`, `tests/tasks/test_outbox_bom_writes_dispatch.py`, `tests/adapters/test_movex_bom_writes.py` |
| I2-21 | **Three real bugs found + fixed via live UAT-prep testing against CONO=300** (2026-08-11): (1) `add_bom_component`/`delete_bom_component` sent lowercase JSON payload keys (`cono`/`mtno`/...) — the generic MI passthrough (`Controllers/TransactionController.cs`'s `ExecuteTransaction` → `TransactionStringBuilder.ResolveFieldValue`) does a case-SENSITIVE field-name match against uppercase real names; every field would have silently failed to resolve. Fixed to uppercase, matching `add_routing_operation`'s already-proven convention — a docstring previously claimed the lowercase convention was intentional, which conflated genuinely-lowercase READ endpoints (B-1/B-2/B-3) with WRITE transaction field names (always uppercase). (2) `delete_bom_component` targeted a nonexistent `DeleteComponent` transaction — the real, only delete transaction is `Delete`. (3) **Critical, universal**: `_run_mi_call`'s success check in `movex_outbox.py` read `response.get("MSID")` at the wrong nesting level — real MSID lives under `response["data"]["MSID"]`, so this check has silently evaluated to `""` (success) for EVERY dispatched MI write since it was written, regardless of what M3 actually returned — meaning **no outbox write, BOM or routing, has ever actually been checked for an M3-side failure**. Fixed to check the real envelope's top-level `"success"` field first (movex-rest-api's own `TransactionController.EvaluateParsedResponse` already does this evaluation server-side), falling back to `data.MSID`/`"000"`-or-empty if `"success"` is absent. All three confirmed via real live calls against CONO=300 (`AddComponent` write + read-back confirmed; `Delete` failed without `FDAT` even for a confirmed-existing `MSEQ`, succeeded once added, removal confirmed via read-back). Also surfaced that `MSEQ` (Sequence Number) is a real, required, separate field from `OPNO` on `AddComponent`/`Delete` — was previously set to `str(operation_number)`; now wired from `ecn_bom_changes.sequence_number` (falls back to `operation_number` for rows authored before this was wired up). | Live UAT-prep verification, 2026-08-11 | `src/adapters/erp/movex.py` (`add_bom_component`, `delete_bom_component`, `update_bom_component`), `src/adapters/erp/base.py`, `src/adapters/erp/ifs.py`, `tests/helpers/fake_erp.py`, `src/services/ecn/workflow.py` (`_queue_bom_changes_outbox` — `sequence_number` now passed through), `src/tasks/movex_outbox.py` (`process_outbox_entry`'s success check) |
| I2-20 | ✅ Bulk BOM-change upload drawer UI — `BOMChangesUploadDrawer.tsx` + `lib/ecn-bom-changes-csv-parser.ts` (mirrors `RoutingUploadDrawer.tsx`/`ecn-routing-csv-parser.ts` exactly) wired to the `Upload BOM Changes` button on `ECNEntityTabsSection.tsx`'s BOM Changes tab. Along the way: `bulk_create_bom_changes` was silently dropping `sequence_number`/`notes`/`circuit_refs_new` on insert even when a caller supplied them (only the single-row `create_bom_change` endpoint persisted all three) — fixed, with a regression test (`test_sequence_number_notes_and_circuit_refs_are_persisted`). Also fixed a query-invalidation gap: `onItemsChanged` never invalidated the `bom-changes`/`ecn-bom-changes-all` query keys, so a successful BOM upload/edit wouldn't refresh either the per-item panel or the aggregate tab without a manual reload. | Slice E delivery gap, closed 2026-08-11 | `frontend/src/components/ecn/BOMChangesUploadDrawer.tsx`, `frontend/src/lib/ecn-bom-changes-csv-parser.ts`, `src/services/ecn/bom_changes.py`, `frontend/src/pages/ECNDetailPage.tsx` |
| I2-22 | **ADR-014 parity gap — `MPDHED` half of the parent-existence check.** The BOM-change parent is validated against `MITMAS` only (via `erp.get_item`); Stargile checked `MITMAS` **and** `MPDHED` (`RequestECNBoMDetailValidationHelper.java:342-352`). A knowing divergence, taken for v1 rather than an oversight — but it means a parent that exists as an item with no product-structure header would pass Oskar's check and could still fail the Movex write. Revisit if that turns out to be reachable in practice; the decision and its reasoning are recorded in ADR-014 and in a comment at `src/routers/ecn_bom.py:86-87`. | ADR-014, 2026-08-25 | `src/routers/ecn_bom.py` |
| I2-23 | **BLOCKED ON movex-rest-api — item search endpoint.** Closes the open half of I2-15 (search-as-you-type item finder) and unblocks the ECN item browser + BOM component picker. There is currently **no endpoint that can serve an item search**: `GET /api/items?q=` (which `MovexRestAdapter.search_items` calls) does not exist — verified against `movex-rest-api/Controllers/`, whose only Db2Query routes are `parts/search-alias`, `parts/next-sequence`, `customers/list`, `bom/*`, `mpm/export`. `MMS200MI.LstItmByNo` is not configured either (`transactions/MMS200MI.json` has GetItmBasic/GetItmFac/LstItmFac/GetItmWhsBal + writes only), so the MI passthrough cannot serve it. `search_items` (`src/adapters/erp/movex.py:470`) is dead speculative code — no docstring, no CONO param (mandatory on multi-company tables), no `data.records` unwrapping; it must be **rewritten**, not merely called. Full spec written for @developer-dotnet: `movex-rest-api/docs/SPEC-item-search-endpoint.md` (DB2 direct query over MITMAS, modelled on `SearchMitpop`; covers per-company schema resolution, prefix-before-substring ranking, `truncated` flag, min-2-char guard, and the unverified status-90 assumption). | ECN authoring UX review, 2026-09-03 | `movex-rest-api/Services/Db2QueryService.cs`, `movex-rest-api/Controllers/Db2QueryController.cs`, then Oskar `src/adapters/erp/movex.py:470`, `src/routers/items.py` |
| I2-24 | ✅ **Test-isolation bug — `test_sse.py` passed or failed on collection order.** Errored at setup with `KeyError: 'MOVEX_API_URL'` (then `'MOVEX_CONO'`) when run alone; passed when any adapter test ran first. `MovexRestAdapter.__init__` reads both via `os.environ[...]` with no default (`src/adapters/erp/movex.py:216,218`), and each adapter test module sets them itself at import time — so any test building the app without doing so depended on another module having leaked them into the process. Fixed 2026-09-03 by moving both to `tests/conftest.py` beside `DATABASE_URL`/JWT secrets, which is what that file exists for. `MOVEX_CONO` defaults to `300` (dev/UAT) — never `100`, which is production per PRE-12. **Separate finding, still open:** `-m "not integration"` excludes nothing — no `integration` marker is registered in `pyproject.toml` and the files under `tests/integration/` do not carry one, so that flag silently matches every test and the DB-backed files run and block (this is the mechanism behind I2-18's "one at a time" workaround). Use `--ignore=tests/integration`; registering a real marker would be the tidier fix. Whole non-integration suite now runs in one invocation: **1409 passed, 5 skipped, 24s**. | Found while verifying the ref-des work, 2026-09-03 | `tests/conftest.py`; marker gap in `pyproject.toml` |
