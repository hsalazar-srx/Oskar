...
| I2-19 | **Live-OQ gate pending** — `MovexRestAdapter.update_bom_component()` (Slice E, W-1/`PDS002MI.UpdateComponent`) ships fully mock-verified against the documented/assumed contract shape only (`docs/movex-rest-api-bom-contract.md`) — W-1 is confirmed not yet built on movex-rest-api as of this writing (see contract doc's checkpoints table). Needs a live-OQ verification pass (real M3 write, field-offset check with the same rigor W-0 got per `analysis/PDS002MI-routing-analysis.md`) once the movex-rest-api owner ships W-1 and its own offset verification — same precedent as S9-7's live-OQ gate for routing writes. Until then, the CHANGE/DELETE BOM-write path (`_queue_bom_changes_outbox` → `update_bom_component`) will fail at the outbox-dispatch stage against a real environment, retried per the standard outbox retry schedule, DC-alerted at attempt 3. | Slice E, R2 risk mitigation, 2026-08-10 | `src/adapters/erp/movex.py` (`update_bom_component`); OQ checklist |
| I2-20 | Bulk BOM-change upload drawer UI — the backend endpoint (`POST /ecn/{ecn_id}/bom-changes/bulk`, ECN-wide/multi-item, verified against the real Stargile `UploadECNBoMs.java` source 2026-08-11) and its API client function (`bulkCreateBomChanges` in `api/ecn.ts`) both exist and are correctly ECN-wide (matching `bulkCreateRoutingOps`'s shape) — but no `BOMChangesUploadDrawer.tsx`/upload button was wired up in this slice, unlike routing's `RoutingUploadDrawer.tsx` + its `Upload Routing` button on `ECNEntityTabsSection.tsx`'s Routing tab. Building the drawer properly means either a new `lib/ecn-bom-changes-csv-parser.ts` (mirroring `ecn-routing-csv-parser.ts`'s client-side pre-validation/preview-table logic, required by the shared `BulkUploadDialog.tsx`'s `parseFile` prop) or a documented, deliberately simpler drawer that skips client-side preview and relies on the backend's existing row validation — not started either way in this slice. | Slice E delivery gap, 2026-08-11 | `frontend/src/components/ecn/` (new `BOMChangesUploadDrawer.tsx` + optional `lib/ecn-bom-changes-csv-parser.ts`); backend + API client already done |
