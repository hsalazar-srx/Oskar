...
POST   /ecn/{ecn_id}/items/{item_id}/routing             Add routing op to an item on the ECN
POST   /ecn/{ecn_id}/routing                              Add routing op naming its product
                                                            directly (ADR-016 — no item needed)
GET    /ecn/{ecn_id}/items/{item_id}/routing             List routing ops for one item
PATCH  /ecn/{ecn_id}/routing/{op_id}                      Update routing op
DELETE /ecn/{ecn_id}/routing/{op_id}                      Remove routing op

  Edit/delete are ECN-scoped only: an operation id is unique on its own, and
  the item-scoped versions took an {item_id} they never read.
...
# ── ADR-016: ECN-scoped routing routes ───────────────────────────────────────
# Same shape as the MPN routes. PATCH and DELETE live HERE ONLY, not under
# /items/{item_id}: an operation id is unique on its own, and _get_routing_op
# no longer uses item_id in its WHERE clause, so an item-scoped copy would run
# byte-identical code behind a path segment nothing reads.
#
# POST keeps both forms because they genuinely differ: with an item, the item
# supplies item_number; without, the caller does.

@ecn_routing_router.post(
    "/{ecn_id}/routing",
    response_model=RoutingOpOut,
    status_code=status.HTTP_201_CREATED,
    summary="Add a routing operation with no item on the ECN (ADR-016)",
)
async def create_standalone_routing_operation(
    ecn_id: str,
    body: StandaloneRoutingOpBody,
    user: Annotated[CurrentUser, Depends(get_current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> RoutingOpOut:
    svc = ECNService(session)
    ...
    try:
        op = await svc.create_routing_operation(
            ecn_id, None, req, item_number=body.item_number
        )
    except ECNNotFound:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="ECN not found")
    except ECNConflict:
...
@ecn_routing_router.patch(
    "/{ecn_id}/routing/{op_id}",
    response_model=RoutingOpOut,
    summary="Update a routing operation, with or without an item (ADR-016)",
)
async def update_routing_operation(
    ...
    try:
        op = await svc.update_routing_operation(
            ecn_id, op_id, **body.model_dump(exclude_none=True)
        )
    except ECNNotFound:
        raise HTTPException(
...
@ecn_routing_router.delete(
    ...
    response_model=None,
    summary="Delete a routing operation, with or without an item (ADR-016)",
)
async def delete_routing_operation(
    ...
    svc = ECNService(session)
    try:
        await svc.delete_routing_operation(ecn_id, op_id)
    except ECNNotFound:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Routing operation not found"
        )
