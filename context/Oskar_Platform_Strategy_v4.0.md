...
## Modernisation and Build Strategy v4.0

**Version:** 4.0
**Status:** Revised Strategy — Full Organisational Context Integration
...
Non-Negotiable #11 is new in v4.0. It codifies the platform-foundation vision as a hard constraint on every design decision, not an aspiration.
...
*(Unchanged from v3.5 Section 7.)*

**One addition from organisational context:** The `ai/01-manufacturing-context.md` file seeded in Phase 0 must include Scanfil Group structure, QSDC framework, Dream Factory roadmap, JB site context, and the list of key stakeholders with their roles. This is not documentation for documentation's sake — it is the context the AI development harness needs to produce suggestions that are organisationally appropriate, not just technically correct.
...
| **Manal** | Infrastructure Manager | Docker on Windows Server; network; ADCS certificates | Phase 2 gate: Docker confirmed, two-site deployment, ADCS certificate |
...
| Frontend | React / TypeScript | Production-grade UI; CBM and Purchasing-appropriate interface |
| Database | PostgreSQL 16 | Enterprise-grade; extensible schema; future Data Warehouse integration point |
| Cache + Event Bus | Redis 7 (cache + Streams) | Dual purpose; Redis Streams feeds Power BI in Phase 4 |
| Background workers | Celery + Redis | Supplier API fan-out; future batch jobs |
...
| Auth | JWT + Active Directory | Engineers use Windows credentials; leavers auto-locked out |
...
On-premise, Windows Server. Docker on Windows Server via WSL2. Two instances — one per site — from same Docker images with different environment files. Manal (Infrastructure Manager) is the named owner for Docker confirmation and ADCS certificate provisioning (Phase 2 gate condition).
...
The strategy is sound. The organisational context is integrated. The plan is ready to execute.
