...
class TestFakeERPAdapterGetBomIndented:
    async def test_returns_multi_level_fixture_for_known_item(self):
        adapter = FakeERPAdapter()
    ...
    async def test_unknown_item_raises_lookup_error(self):
        ...
        with pytest.raises(LookupError):
            await adapter.get_bom_indented("NOPE99999", "D")


class TestFakeERPAdapterGetWhereUsed:
    async def test_returns_where_used_fixture_for_known_component(self):
        adapter = FakeERPAdapter()
    ...
    async def test_unknown_component_raises_lookup_error(self):
        adapter = FakeERPAdapter()

        with pytest.raises(LookupError):
            await adapter.get_where_used("NOPE99999", "D")
